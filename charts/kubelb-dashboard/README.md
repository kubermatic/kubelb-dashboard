...
```bash
helm upgrade kubelb-dashboard oci://quay.io/kubermatic/helm-charts/kubelb-dashboard --version 0.0.0-dev --namespace kubelb --create-namespace --install
```
...
## Example with HTTPRoute (Gateway API)
...
Requires the Gateway API v1 CRDs (`gateway.networking.k8s.io/v1`) installed in the cluster.
...
| httpRoute.enabled | bool | `false` | Enable HTTPRoute (Gateway API v1). Independent of `ingress.enabled` — both may be on simultaneously. |
| httpRoute.hostnames | list | `[]` | HTTPRoute hostnames |
| httpRoute.parentRefs | list | `[]` | HTTPRoute parentRefs. Each entry: {name, namespace?, sectionName?, port?} |
| httpRoute.rules | list | `[]` | Override default routing rules. If empty, a single rule routes PathPrefix `/` to the chart's Service on `service.port`. |
...
| metrics.autoDiscover | bool | `true` | Auto-discover Prometheus when prometheusUrl is empty (explicit URL always wins) |
| metrics.prometheusUrl | string | `""` | Prometheus base URL, e.g. http://prometheus.monitoring.svc:9090 |
...
| readOnly | bool | `false` | Run the dashboard in read-only mode. The API server rejects mutating requests to the Kubernetes API and the UI hides all create/edit/delete controls. When `rbac.create` is true, the ClusterRole is also narrowed to read-only verbs (defense-in-depth). |
...
| traffic.autoDiscover | bool | `true` | Auto-discover Hubble Relay when hubbleRelayAddress is empty (explicit address always wins) |
| traffic.hubbleRelayAddress | string | `""` | Hubble Relay address, e.g. hubble-relay.kube-system.svc:80 |
| watch.enabled | bool | `false` | Enable Kubernetes watch streaming for resource lists |
