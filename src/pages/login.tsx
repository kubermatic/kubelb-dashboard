...
export function LoginPage() {
  ...
  return (
    <div
      className="relative min-h-screen overflow-hidden bg-[#0c0e14] font-sans outline-none"
    ...
    >
      ...
      <svg className="pointer-events-none absolute inset-0 h-full w-full opacity-[0.06]">
        <defs>
          <pattern id="kubelb-grid" width="40" height="40" patternUnits="userSpaceOnUse">
            <path d="M 40 0 L 0 0 0 40" fill="none" stroke="#4fd1c5" strokeWidth="0.5" />
          </pattern>
        </defs>
      ...
      <div className="flex min-h-screen flex-col items-center justify-center pb-16 mx-auto max-w-[420px] px-6">
        ...
        <div className="mb-8 text-center">
          ...
          <h1 className="mb-1.5 text-[28px] font-medium tracking-tight text-[#f7fafc]">KubeLB</h1>
          <p className="text-sm font-normal tracking-[1.5px] text-[#4fd1c5]">
        ...
        <div className="mb-8 flex flex-wrap justify-center gap-2">
          {FEATURE_PILLS.map((label) => (
            <span
              key={label}
              className="rounded-full border border-[rgba(79,209,197,0.2)] bg-[rgba(79,209,197,0.1)] px-3.5 py-1 text-xs text-[#81e6d9]"
            >
              {label}
            </span>
          ))}
        ...
        <div className="mb-4 text-center">
          <button
            onClick={handleLogin}
            className="inline-flex cursor-pointer items-center gap-2.5 rounded-lg border-none bg-[#4fd1c5] px-9 py-3 text-[15px] font-medium tracking-wide text-[#0c0e14] transition-colors hover:bg-[#38b2ac]"
          >
            Sign In
          </button>
        </div>
        <p className="mb-2 text-center text-xs text-[#4a5568]">
          Authenticate via your identity provider
        </p>
        <p className="text-center text-[11px] text-[#2d3748]">
          Press{" "}
          <kbd className="rounded border border-[#2d3748] bg-[#1a202c] px-1.5 py-px text-[10px] text-[#718096]">
            Enter ↵
          </kbd>{" "}
          to sign in
        </p>
      ...
      {/* Footer */}
      <div className="absolute inset-x-0 bottom-0 flex items-center justify-center gap-6 border-t border-[rgba(255,255,255,0.04)] px-8 py-5">
        <a
