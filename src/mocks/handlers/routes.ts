...
import { routes as seed } from "../fixtures";
import { kubeListEnvelope, kubeStatus } from "../helpers";
import { MockStore } from "../store";

const store = new MockStore<Route>(seed);
...
export const routeHandlers = [
  ...
  http.get(`${API}/namespaces/:namespace/routes/:name`, ({ params }) => {
    const item = store.get(params.name as string, params.namespace as string);
    if (!item) {
      return HttpResponse.json(
        kubeStatus(404, "NotFound", `routes "${params.name as string}" not found`),
        { status: 404 },
      );
    }
